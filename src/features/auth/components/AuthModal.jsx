...
export const fetchSessionsLLM = createAsyncThunk('chat/fetchSessions', async () => {
  const response = await apiClient.get(endpoints.sessions.list_llm);
  return response.data;
});

export const fetchSessionsASR = createAsyncThunk('asrChat/fetchSessions', async () => {
  const response = await apiClient.get(endpoints.sessions.list_asr);
  return response.data;
});

export const fetchSessionsTTS = createAsyncThunk('ttsChat/fetchSessions', async () => {
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl shadow-lg max-w-md w-full p-6 sm:p-8 relative max-h-[90vh] overflow-y-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-xl font-semibold text-gray-900 mb-2">{isAnonymous ? 'Sign in Now' : 'Sign in to unlock all features'}</h2>
            <p className="text-sm text-gray-500">Get persistent chat history and access to all models</p>
          </div>
          ...
          {!showPhoneAuth ? (
            <div className="space-y-3">
              ...
              {isAnonymous && <p className="text-xs text-gray-400 text-center pt-2">Your current session will be transferred to your account</p>}
            </div>
          ) : (
            <div className="space-y-4">
              <PhoneAuth onSuccess={handlePhoneAuthSuccess} onCancel={() => setShowPhoneAuth(false)} />
            </div>
          )}
