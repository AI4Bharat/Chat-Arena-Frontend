...
export function TopContributors({ type, defaultLanguage = 'en', defaultOrganization = 'ai4b', languageOptions = [], organizationOptions = [] }) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((row) => row.user?.toLowerCase().includes(q) || row.email?.toLowerCase().includes(q));
    }
  ...
  return (
    <div className="flex-1 overflow-y-auto min-h-[80vh] bg-gray-50">
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        <div className="mb-6">
          <div className="flex flex-col lg:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div className="flex-1">
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Top Contributors</h1>
              <p className="text-gray-600 text-xs max-w-lg md:text-sm">Recognizing our top contributors based on chat sessions and votes.</p>
            </div>
        ...
        {/* Table */}
        <LeaderboardTable data={filteredData} columns={columns} compact={false} loading={loading} emptyMessage={searchQuery ? 'No contributors found matching your search' : 'No contributors found'} />
      </div>
    </div>
  );
}
