...
export function LeaderboardTable({ data = [], columns = [], onRowClick, rowKey = 'id', compact = false, viewAllLink = null, loading = false, emptyMessage = null }) {
            ...
            {loading ? (
              ...
              sortedData.map((row, i) => (
                <tr
                  ...
                  className={`border-b border-gray-100 hover:bg-gray-50 transition-colors bg-white ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} ${onRowClick ? 'cursor-pointer' : ''}`}
                >
                  {columns.map((col) => (
                    <td key={`${row[rowKey] || i}-${col.key}`} className={`px-4 py-3 text-gray-900 text-sm ${col.align === 'right' ? 'text-right' : 'text-left'} ${col.cellClassName || ''}`}>
