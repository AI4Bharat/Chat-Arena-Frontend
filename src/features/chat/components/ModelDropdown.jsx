...
export function ModelDropdown({ models, selectedModelId, onSelect, disabled = false, fullWidth = false }) {
  ...
  const filteredModels = models.filter((model) => model.display_name.toLowerCase().includes(searchTerm.toLowerCase()) || model.provider.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className={`relative ${containerWidthClass}`} ref={wrapperRef} data-tour="model-selector">
      ...
      >
        ...
        {isSelectedMultimodal && <Paperclip size={14} className="text-green-600 shrink-0 ml-1" title="Supports attachments (Images, Docs, Audio)" />}
              ...
              return (
                ...
                >
                  <span className="flex items-center gap-2 sm:whitespace-nowrap">
                    ...
                    {isMultimodal && <Paperclip size={14} className="text-green-600 shrink-0" title="Supports attachments (Images, Docs, Audio)" />}
                  </span>
