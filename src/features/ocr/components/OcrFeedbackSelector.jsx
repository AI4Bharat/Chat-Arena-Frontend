...
export function OcrFeedbackSelector({ onSelect, submitted, winner }) {
  if (submitted) {
    ...
    return (
      ...
      >
        <span className="text-sm font-medium text-green-600 bg-green-50 border border-green-200 rounded-full px-4 py-1.5">
          Submitted: {label}
        </span>
      </motion.div>
    );
  ...
  return (
    <motion.div
      ...
      className="flex justify-center py-3"
    >
      <div className="flex items-center p-1 bg-white/90 backdrop-blur border border-gray-200/80 rounded-full shadow-md gap-1">
        {/* Mobile: icon-only */}
