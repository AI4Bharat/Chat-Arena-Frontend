...
import { TranslateIconThai } from '../../../shared/icons/TranslateIconAquarium';
import { LanguageSelector } from './LanguageSelectorAquarium';
import { PrivacyNotice } from './PrivacyNoticeAquarium';
...
export function MessageInput({ sessionId, modelAId, modelBId, isCentered = false, isSidebarOpen = true, onInputActivityChange }) {
      ...
      <>
        <div className={`w-full px-2 sm:px-4 ${isCentered ? 'pb-0' : 'pb-2 sm:pb-4'} bg-transparent`}>
          <form onSubmit={handleSubmit} className={`relative ${formMaxWidth}`}>
            ...
            <div className={`relative flex items-center justify-between bg-white border-2 border-blue-500 rounded-xl shadow-sm w-full p-3`}>
  ...
  return (
    <>
      <div className={`w-full px-2 sm:px-4 ${isCentered ? 'pb-0' : 'pb-2 sm:pb-4'} bg-transparent`}>
        <form onSubmit={handleSubmit} className={`relative ${formMaxWidth}`}>
          <div className={`relative flex flex-col bg-white border-2 border-blue-500 rounded-xl shadow-sm w-full`} data-tour="tts-message-input">
            <IndicTransliterate
              ...
              suggestionListClassName="
                ...
                bg-white border border-blue-200 rounded-lg shadow-xl
              ...
              activeSuggestionItemClassName="
                px-3 py-2 rounded-md text-sm text-white bg-blue-500 w-full text-center sm:w-auto sm:text-left
            ...
            <div className="flex items-center justify-between p-2">
              <div className="flex items-center">
                <button
                  ...
                  className={`p-1.5 sm:p-2 rounded-md transition-colors disabled:opacity-50 ${isTranslateEnabled ? 'text-blue-500 hover:bg-blue-50' : 'text-gray-500 hover:bg-gray-100'}`}
                  aria-label="Toggle Transliteration"
                  title={isTranslateEnabled ? 'Switch to English' : 'Switch to Singaporean Languages'}
                >
                  {isTranslateEnabled ? <TranslateIconThai className="h-5 w-5 sm:h-6 sm:w-6" fill='#264CC7' /> : <TranslateIconThai className="h-5 w-5 sm:h-6 sm:w-6" />}
                </button>
              ...
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  ref={micButtonRef}
                  className={`p-1.5 sm:p-2 text-gray-500 rounded-md hover:bg-gray-100 hover:text-blue-600 transition-colors disabled:opacity-50`}
                  aria-label="Voice Typing"
                  title="Voice Typing"
                >
                  {voiceState === 'loading' ? (
                    <LoaderCircle size={18} className="text-blue-500 animate-spin sm:w-5 sm:h-5" />
                  ) : voiceState === 'recording' ? (
                    <div className="flex items-center justify-center gap-0.5 w-5 h-5">
                      <span className="inline-block w-0.5 h-3 bg-blue-500 rounded-full animate-sound-wave"></span>
                      <span className="inline-block w-0.5 h-4 bg-blue-500 rounded-full animate-sound-wave [animation-delay:100ms]"></span>
                      <span className="inline-block w-0.5 h-2 bg-blue-500 rounded-full animate-sound-wave [animation-delay:200ms]"></span>
                      <span className="inline-block w-0.5 h-3.5 bg-blue-500 rounded-full animate-sound-wave [animation-delay:300ms]"></span>
                      <span className="inline-block w-0.5 h-2.5 bg-blue-500 rounded-full animate-sound-wave [animation-delay:400ms]"></span>
                    </div>
                  ) : (
