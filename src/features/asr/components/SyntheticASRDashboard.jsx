...
function JobRow({ job, navigate, onRefresh, onEditDraft }) {
    ...
    const handleResubmit = (e) => {
        ...
        const fd = p._wizard_form_data || {
            ...
            language: p.sentence?.language || p.language || '',
            ...
            subDomains: p.sub_domains || [],
            personas: (p.personas || []).map(item => ({
                topic: item.topic,
                persona: item.persona,
                subDomain: item.subDomain || item.sub_domain || ''
            })),
            situations: p.scenarios || [],
            sentences: (p.sentence?.list || []).map(s => typeof s === 'string' ? s : (s.sentence || '')),
            audioConfig: p.audio ? {
        ...
        onEditDraft({ formData: fd, currentStage: 1, isResubmit: true, isCompletedDraft: isReady });
    };
