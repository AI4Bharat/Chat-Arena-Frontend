...
import { Plus, ChevronDown, Search, Eye } from 'lucide-react';
...
export function SyntheticASRDashboard({ onCreateNewClick }) {
    ...
    // DEMO: Always include the completed demo job so visualization can be tested
    const completedDemoJob = demoJobs[0]; // The COMPLETED job with ID: demo-2026-01-30-001
    const baseJobs = error && jobs.length === 0 ? demoJobs : [completedDemoJob, ...jobs];
...
function JobRow({ job, getRelativeTime, getStatusColor, getStatusIcon, getProgressBarColor, getStatusLabel, navigate }) {
        ...
        <div className="bg-white border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            >
                ...
                <div className="ml-4 flex items-center gap-4 min-w-0">
                    ...
                    {/* View Visualization Button (Only for COMPLETED jobs) */}
                    {upperStatus === 'COMPLETED' && (
                        <button
                            ...
                            title="View Audio Visualization"
                        >
                            <Eye size={20} />
                        </button>
                    )}
