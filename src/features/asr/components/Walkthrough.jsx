...
export function Walkthrough() {
  ...
  const steps = [
    ...
    },
    {
      ...
      title: 'Your Sidebar',
      content: <p className="text-gray-700 text-sm leading-relaxed">Access your ASR history, start a new session, or view the Leaderboard from here.</p>,
      placement: 'right',
      avoidCovering: true,
    },
    {
      ...
      content: <p className="text-gray-700 text-sm leading-relaxed">Click here anytime to start a fresh ASR session with new audio.</p>,
      placement: 'right',
      avoidCovering: true,
    },
    {
      ...
      content: <p className="text-gray-700 text-sm leading-relaxed">See which ASR models are performing the best based on user feedback and community votes.</p>,
      placement: 'bottom',
      avoidCovering: true,
    },
    {
    ...
    },
    {
      ...
      content: <p className="text-gray-700 text-sm leading-relaxed">Choose your preferred Indian language for speech recognition. Different models support different languages.</p>,
      placement: 'top',
      optional: true,
    },
  ];
  ...
  useEffect(() => {
    ...
    if (prevAutoOpened) {
      const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
      const needsSidebar =
        currentStep >= 0 && currentStep < steps.length && ['[data-tour="asr-sidebar"]', '[data-tour="asr-new-chat"]', '[data-tour="asr-leaderboard-link"]'].includes(steps[currentStep].target);
      if (isMobile && !needsSidebar) {
        closeSidebarMobile();
  ...
  const forceCenteredArrow = !isCentered && isDesktop && orientation === 'bottom' && (step.target === '[data-tour="asr-mode-selector"]' || step.target === '[data-tour="asr-leaderboard-link"]');
