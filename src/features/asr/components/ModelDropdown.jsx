  ...
  return (
    <div className={`relative ${containerWidthClass}`} ref={wrapperRef}>
      <button
        ...
        disabled={disabled}
        className="w-full text-left flex items-center justify-between p-2.5 rounded-md 
  hover:bg-gray-100 dark:hover:bg-[#444444] dark:hover:bg-[#444444] transition-colors
  ${selectedModelId === model.id ? 'bg-orange-50 dark:bg-[#3a2b1f]' : ''}"
      >
...
<ChevronDown size={16} className={`transition-transform duration-200 text-gray-500 dark:text-[#a0a0a0] ml-auto shrink-0 ${isOpen ? 'rotate-180' : ''}`} />      </button>

      {isOpen && (
        <div 
          className="absolute top-full mt-2 min-w-full w-48 sm:w-max max-w-sm bg-white dark:bg-[#2a2a2a] border border-gray-200 dark:border-[#3a3a3a] rounded-lg shadow-xl z-20 origin-top transition-all duration-200 ease-out opacity-100 scale-100 left-1/2 -translate-x-1/2"
        >
          <div className="p-2 border-b border-gray-200 dark:border-[#3a3a3a]">
            <div className="relative">
              <Search size={18} className="w-full bg-gray-50 dark:bg-[#333333] text-gray-800 dark:text-[#ececec] placeholder:text-gray-400 dark:placeholder:text-[#a0a0a0] border border-gray-200 dark:border-[#3a3a3a] rounded-md pl-10 pr-3 py-2 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400" />
              <input
                type="text"
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            ...
            return (
              <button
                ...
                className={`w-full text-left flex items-center justify-between p-2.5 rounded-md hover:bg-gray-100 dark:hover:bg-[#444444] transition-colors ${selectedModelId === model.id ? 'bg-orange-50 dark:bg-[#3a2b1f]' : ''}`}
              >
